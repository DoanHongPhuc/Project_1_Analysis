...
function StudentList() {
  ...
  const getReportData = async () => {
    const url = process.env.REACT_APP_API_KEY + `/student/list/`;
    const data = {
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        setNumPage(Math.floor(result.data.number_student / size));
      }
      setLoading(false);
    } catch (e) {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <a
            onClick={() => {
              updateStuEmail(record.Email);
              navigate("/student_report");
            }}
            className="text-[#0086ff]"
          >
            Xem báo cáo
          </a>
        </Space>
